...
import { Card, Color, DeckFormat, DeckModel, Recommendation } from "@/lib/types";
...
export function DeckBuilder() {
  ...
  useEffect(() => {
    void searchCards();
  }, [filters, deck.commanderId]);

  useEffect(() => {
    void loadRecommendations();
  }, [deck.entries, deck.commanderId, deck.format]);
  ...
  async function searchCards() {
    ...
    const commanderIdentity = deck.commanderId
      ? CARDS.find((card) => card.id === deck.commanderId)?.colorIdentity.join(",")
      : "";
  ...
  async function loadDecks() {
    ...
    if (data.decks.length && !activeDeckId) {
      const first = data.decks[0] as DeckModel;
      setDeck(first);
  ...
  async function loadRecommendations() {
    const response = await fetch("/api/recommendations", {
      ...
      body: JSON.stringify({ entries: deck.entries, commanderId: deck.commanderId, format: deck.format })
    });
    const data = await response.json();
  ...
  async function duplicateDeck() {
    ...
    await fetch(`/api/decks/${activeDeckId}/duplicate`, { method: "POST" });
        ...
        <div className="mt-3 max-h-[28rem] space-y-2 overflow-auto">
          {cards.map((card) => (
            <article key={card.id} className="rounded border p-2">
              ...
              {card.cardFaces?.length ? <p className="mt-1 text-xs text-slate-500">DFC: {card.cardFaces[1].name}</p> : null}
            </article>
          ))}
        </div>
      </section>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(event) =>
              setDeck((current) => ({
                ...
                minDeckSize: event.target.value === "constructed" ? 60 : 100,
                sideboardLimit: event.target.value === "constructed" ? 15 : 0
              }))
            }
          >
          ...
          {deck.format === "commander" ? (
            ...
            >
              ...
              {CARDS
                .filter((card) => card.typeLine.includes("Legendary") && card.typeLine.includes("Creature"))
                .map((card) => (
