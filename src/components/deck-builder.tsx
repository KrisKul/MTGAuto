...
import { Card, Color, ConstructedFormat, DeckFormat, DeckModel, Recommendation } from "@/lib/types";
...
export function DeckBuilder() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [deck.entries, deck.commanderId, deck.format, deck.constructedFormat]);
          ...
          {cards.map((card) => (
            <article key={card.id} className="rounded border p-2">
              <div className="flex items-start gap-2">
                {card.imageUri ? (
                  <img src={card.imageUri} alt={card.name} className="h-20 w-14 rounded object-cover" loading="lazy" />
                ) : (
