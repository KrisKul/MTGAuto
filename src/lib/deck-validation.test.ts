...
describe("validateDeck", () => {
  ...
  it("warns when commander deck is excessively land-heavy", () => {
    const issues = validateDeck({
      format: "commander",
      commanderId: "infamous-cruelclaw",
      entries: [
        { cardId: "swamp", quantity: 50 },
        { cardId: "mountain", quantity: 49 }
      ]
    });

    expect(issues.some((x) => x.message.includes("strongest around"))).toBe(true);
  });
